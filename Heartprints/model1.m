...
% sinus_intervals = sinus_intervals(sinus_intervals>=theta);

% sum_record_length = sum(sinus_intervals);          % 84875 s , 23.5764 h -> almost 24 h
...
%% Model 1: Random ectopic beats

record_length = 24*3600;         
p = 0.5;                                        % probability for an ectopic beat
...
% sinus_times_inRefrac = false(size(sinus_times));

for i = 1:numel(ectopic_times)
    ectopic_times_inRefrac(i) = any(ectopic_times(i)>=sinus_times & ectopic_times(i)<=sinus_times+theta);
end

% for i = 1:numel(sinus_times)
%     sinus_times_inRefrac(i) = any(sinus_times(i)>=ectopic_times & sinus_times(i)<=ectopic_times+theta);
% end

% Remove elements if they fall within the ranges
ectopic_times(ectopic_times_inRefrac) = [];
% sinus_times(sinus_times_inRefrac) = [];
...
%     end
% end
CI=arrayfun(@(et) et -sinus_times(find(sinus_times< et, 1, 'last')), ectopic_times);
